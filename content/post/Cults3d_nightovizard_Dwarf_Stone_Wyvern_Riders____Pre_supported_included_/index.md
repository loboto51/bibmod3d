
---
title: "[GRATUIT] Cults3d > nightovizard > Dwarf_Stone_Wyvern_Riders____Pre_supported_included_ [non dl]"
date: "2022-04-25T17:54:02Z"
toc: true
image: "thumbs_-_dwarf-stone-wyvern-riders-pre-supported-included_-_Images_1.jpg"
tags:
- "nightovizard"
- "28mm"
- "NAINS"
- "28mm NAINS"
- "NONDL"
- "28mm NONDL"
- "NAINS NONDL"
- "28mm NAINS NONDL"
- "SUP"
- "28mm SUP"
- "NAINS SUP"
- "28mm NAINS SUP"
- "NONDL SUP"
- "28mm NONDL SUP"
- "NAINS NONDL SUP"
- "28mm NAINS NONDL SUP"
- "warmaster"
- "Bits"
- "warmaster Bits"
- "Bits-creatures-geantes"
- "warmaster Bits-creatures-geantes"
- "Bits Bits-creatures-geantes"
- "warmaster Bits Bits-creatures-geantes"
- "NONDL"
- "warmaster NONDL"
- "Bits NONDL"
- "warmaster Bits NONDL"
- "Bits-creatures-geantes NONDL"
- "warmaster Bits-creatures-geantes NONDL"
- "Bits Bits-creatures-geantes NONDL"
- "warmaster Bits Bits-creatures-geantes NONDL"
- "SUP"
- "warmaster SUP"
- "Bits SUP"
- "warmaster Bits SUP"
- "Bits-creatures-geantes SUP"
- "warmaster Bits-creatures-geantes SUP"
- "Bits Bits-creatures-geantes SUP"
- "warmaster Bits Bits-creatures-geantes SUP"
- "NONDL SUP"
- "warmaster NONDL SUP"
- "Bits NONDL SUP"
- "warmaster Bits NONDL SUP"
- "Bits-creatures-geantes NONDL SUP"
- "warmaster Bits-creatures-geantes NONDL SUP"
- "Bits Bits-creatures-geantes NONDL SUP"

---

![](<thumbs_-_dwarf-stone-wyvern-riders-pre-supported-included_-_Images_1.jpg> "thumbs_-_dwarf-stone-wyvern-riders-pre-supported-included_-_Images_1.jpg")

  ![](<thumbs_-_dwarf-stone-wyvern-riders-pre-supported-included_-_Makes_1.jpg> "thumbs_-_dwarf-stone-wyvern-riders-pre-supported-included_-_Makes_1.jpg")

  

Source : [https://cults3d.com/en/3d-model/game/dwarf-stone-wyvern-riders-pre-supported-included](https://cults3d.com/en/3d-model/game/dwarf-stone-wyvern-riders-pre-supported-included)

Id objet : dwarf-stone-wyvern-riders-pre-supported-included

Prix : 0.0 euros

Collections :

- 28mm NAINS
- warmaster Bits Bits-creatures-geantes


Collections pour tags :

- 28mm NAINS NONDL SUP
- warmaster Bits Bits-creatures-geantes NONDL SUP


Description :

~~~
Who says the dwarf army cannot benefit from having creatures when they already have machines?

I decided to go and make that happen, so if you want an alternative to Dwarf Gyrocopters and Bombers that are not a machine, you definitely can.

This features two 28mm Wyvern and rider poses, with an optional set of legs holding a barrel representing a bomb.For miniature wargames, Kings of War, 9th Age or whatever fantasy setting you want.

They fit well in 40 mm bases or bigger. Supported and non-supported version included. For assembly, check the pictures. I recommend a size increase of 15%.
~~~

Justification du tag SUP :

They fit well in 40 mm bases or bigger. Supported and non-supported version included. For assembly, check the pictures. I recommend a size increase of 15%.





