
---
title: "[GRATUIT] Cults3d > Compound > various_bodies_for_old_windriders [dl]"
date: "2024-11-22T07:28:14Z"
toc: true
image: "thumbs_-_various-bodies-for-old-windriders_-_Images_1.avif"
tags:
- "Compound"
- "28mm"
- "ELDARS"
- "28mm ELDARS"
- "VM"
- "28mm VM"
- "ELDARS VM"
- "28mm ELDARS VM"
- "SUP"
- "28mm SUP"
- "ELDARS SUP"
- "28mm ELDARS SUP"
- "VM SUP"
- "28mm VM SUP"
- "ELDARS VM SUP"

---

![](<thumbs_-_various-bodies-for-old-windriders_-_Images_1.avif> "thumbs_-_various-bodies-for-old-windriders_-_Images_1.avif")

  ![](<thumbs_-_various-bodies-for-old-windriders_-_ImagesSTL_1.avif> "thumbs_-_various-bodies-for-old-windriders_-_ImagesSTL_1.avif")

  

Source : [https://cults3d.com/en/3d-model/game/various-bodies-for-old-windriders](https://cults3d.com/en/3d-model/game/various-bodies-for-old-windriders)

Id objet : various-bodies-for-old-windriders

Prix : 0.0 euros

Collections :

- 28mm ELDARS VM


Collections pour tags :

- 28mm ELDARS VM  SUP


Description :

~~~
Old space elf biker bodies with modern helmets in male and female.
Might work for converting modern space elf bikers.
Supported and unsupported. 
Supports are untested but should work.

UPDATE!!
the bodies work, but needed some fitting, this update addresses that, and adds a few other helmet/torso combos. bodies should slip into the seat with minimal or no fitting.
new parts are supported and unsupported, and can be broken down in blender using "separate by loose parts".

my supports are untested and probably overkill. They may also be underkill in some spots; I tried to stick to 0.12mm to reduce scarring.
~~~

Justification du tag SUP :

Supported and unsupported. 
new parts are supported and unsupported, and can be broken down in blender using "separate by loose parts".
ImagesSTL_-_WINDRIDER_zip_unpack_Supported__PILOT_1__stl_.jpg
ImagesSTL_-_WINDRIDER_zip_unpack_Supported__PILOT_2__stl_.jpg
ImagesSTL_-_WINDRIDER_zip_unpack_Supported__PILOT_3__stl_.jpg
ImagesSTL_-_WINDRIDER_zip_unpack_Supported__PILOT_4__stl_.jpg
ImagesSTL_-_WINDRIDER_zip_unpack_Unsupported_PILOT_1_stl_.jpg
ImagesSTL_-_WINDRIDER_zip_unpack_Unsupported_PILOT_2_stl_.jpg
ImagesSTL_-_WINDRIDER_zip_unpack_Unsupported_PILOT_3_stl_.jpg
ImagesSTL_-_WINDRIDER_zip_unpack_Unsupported_PILOT_4_stl_.jpg




