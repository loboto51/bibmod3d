
---
title: "[GRATUIT] Cults3d_supprimes_stl > jimmyzimms > Galactic_Crusader_Weapon_Asset_Pack_Remaster [dl]"
date: "2024-02-14T14:09:20Z"
toc: true
image: "thumbs_-_galactic-crusader-weapon-asset-pack-remaster-jimmyzimms_-_Images_1.jpg"
tags:
- "jimmyzimms"
- "epic"
- "infanterie"
- "epic infanterie"
- "SOB"
- "epic SOB"
- "infanterie SOB"
- "epic infanterie SOB"
- "Bits"
- "epic Bits"
- "infanterie Bits"
- "epic infanterie Bits"
- "SOB Bits"
- "epic SOB Bits"
- "infanterie SOB Bits"
- "epic infanterie SOB Bits"
- "NONSUP"
- "epic NONSUP"
- "infanterie NONSUP"
- "epic infanterie NONSUP"
- "SOB NONSUP"
- "epic SOB NONSUP"
- "infanterie SOB NONSUP"
- "epic infanterie SOB NONSUP"
- "Bits NONSUP"
- "epic Bits NONSUP"
- "infanterie Bits NONSUP"
- "epic infanterie Bits NONSUP"
- "SOB Bits NONSUP"
- "epic SOB Bits NONSUP"
- "infanterie SOB Bits NONSUP"
- "epic"
- "infanterie"
- "epic infanterie"
- "SQUATS"
- "epic SQUATS"
- "infanterie SQUATS"
- "epic infanterie SQUATS"
- "Bits"
- "epic Bits"
- "infanterie Bits"
- "epic infanterie Bits"
- "SQUATS Bits"
- "epic SQUATS Bits"
- "infanterie SQUATS Bits"
- "epic infanterie SQUATS Bits"
- "NONSUP"
- "epic NONSUP"
- "infanterie NONSUP"
- "epic infanterie NONSUP"
- "SQUATS NONSUP"
- "epic SQUATS NONSUP"
- "infanterie SQUATS NONSUP"
- "epic infanterie SQUATS NONSUP"
- "Bits NONSUP"
- "epic Bits NONSUP"
- "infanterie Bits NONSUP"
- "epic infanterie Bits NONSUP"
- "SQUATS Bits NONSUP"
- "epic SQUATS Bits NONSUP"
- "infanterie SQUATS Bits NONSUP"
- "epic"
- "SMI"
- "epic SMI"
- "Bits"
- "epic Bits"
- "SMI Bits"
- "epic SMI Bits"
- "Builders"
- "epic Builders"
- "SMI Builders"
- "epic SMI Builders"
- "Bits Builders"
- "epic Bits Builders"
- "SMI Bits Builders"
- "epic SMI Bits Builders"
- "NONSUP"
- "epic NONSUP"
- "SMI NONSUP"
- "epic SMI NONSUP"
- "Bits NONSUP"
- "epic Bits NONSUP"
- "SMI Bits NONSUP"
- "epic SMI Bits NONSUP"
- "Builders NONSUP"
- "epic Builders NONSUP"
- "SMI Builders NONSUP"
- "epic SMI Builders NONSUP"
- "Bits Builders NONSUP"
- "epic Bits Builders NONSUP"
- "SMI Bits Builders NONSUP"

---

![](<thumbs_-_galactic-crusader-weapon-asset-pack-remaster-jimmyzimms_-_Images_1.jpg> "thumbs_-_galactic-crusader-weapon-asset-pack-remaster-jimmyzimms_-_Images_1.jpg")

  ![](<thumbs_-_galactic-crusader-weapon-asset-pack-remaster-jimmyzimms_-_ImagesSTL_1.jpg> "thumbs_-_galactic-crusader-weapon-asset-pack-remaster-jimmyzimms_-_ImagesSTL_1.jpg")

  

Source : [https://cults3d.com/en/3d-model/game/galactic-crusader-weapon-asset-pack-remaster-jimmyzimms](https://cults3d.com/en/3d-model/game/galactic-crusader-weapon-asset-pack-remaster-jimmyzimms)

Id objet : galactic-crusader-weapon-asset-pack-remaster-jimmyzimms

Prix : 0 euros

Collections :

- epic infanterie SOB Bits
- epic infanterie SQUATS Bits
- epic SMI Bits Builders


Collections pour tags :

- epic infanterie SOB Bits  NONSUP
- epic infanterie SQUATS Bits  NONSUP
- epic SMI Bits Builders  NONSUP


Description :

~~~
This package contains the Galactic Crusader weapon and gear parts rebuilt from the ground up to clean up and correct the mesh errors and normalize the parts commonly used in remixes. All parts have pivot points and reasonable origins. In addition, the objects in the blend are all have been marked as assets allow you to drop this file into your blender asset folder and you will now have all the Galactic Crusader gear ready for your use in your custom remixes. 

Using designs originally from Moonjammy, KMFP, Jimmyzimms, Mildseven, samdathing, DaffMakes, and speacock

Understanding Blender Asset Libraries:
https://docs.blender.org/manual/en/latest/files/asset_libraries/index.html
https://www.youtube.com/watch?v=ERj1m1zBmrs
https://www.youtube.com/watch?v=txSvPyV3FUo
~~~




