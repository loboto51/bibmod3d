
---
title: "[GRATUIT] Cults3d_supprimes_stl > NyanCommissar > 8mm_Space_Elves_Macrowraith_Pack [dl]"
date: "2024-05-13T09:18:21Z"
toc: true
image: "thumbs_-_8mm-space-elves-macrowraith-pack_-_Images_1.jpg"
tags:
- "NyanCommissar"
- "epic"
- "marcheurs"
- "epic marcheurs"
- "VM"
- "epic VM"
- "marcheurs VM"
- "epic marcheurs VM"
- "Eldars-Seigneurs-Fantômes"
- "epic Eldars-Seigneurs-Fantômes"
- "marcheurs Eldars-Seigneurs-Fantômes"
- "epic marcheurs Eldars-Seigneurs-Fantômes"
- "VM Eldars-Seigneurs-Fantômes"
- "epic VM Eldars-Seigneurs-Fantômes"
- "marcheurs VM Eldars-Seigneurs-Fantômes"
- "epic marcheurs VM Eldars-Seigneurs-Fantômes"
- "SUP"
- "epic SUP"
- "marcheurs SUP"
- "epic marcheurs SUP"
- "VM SUP"
- "epic VM SUP"
- "marcheurs VM SUP"
- "epic marcheurs VM SUP"
- "Eldars-Seigneurs-Fantômes SUP"
- "epic Eldars-Seigneurs-Fantômes SUP"
- "marcheurs Eldars-Seigneurs-Fantômes SUP"
- "epic marcheurs Eldars-Seigneurs-Fantômes SUP"
- "VM Eldars-Seigneurs-Fantômes SUP"
- "epic VM Eldars-Seigneurs-Fantômes SUP"
- "marcheurs VM Eldars-Seigneurs-Fantômes SUP"

---

![](<thumbs_-_8mm-space-elves-macrowraith-pack_-_Images_1.jpg> "thumbs_-_8mm-space-elves-macrowraith-pack_-_Images_1.jpg")

  ![](<thumbs_-_8mm-space-elves-macrowraith-pack_-_ImagesSTL_1.jpg> "thumbs_-_8mm-space-elves-macrowraith-pack_-_ImagesSTL_1.jpg")

  

Source : [https://cults3d.com/en/3d-model/game/8mm-space-elves-macrowraith-pack](https://cults3d.com/en/3d-model/game/8mm-space-elves-macrowraith-pack)

Id objet : 8mm-space-elves-macrowraith-pack

Prix : 0 euros

Collections :

- epic marcheurs VM Eldars-Seigneurs-Fantômes


Collections pour tags :

- epic marcheurs VM Eldars-Seigneurs-Fantômes  SUP


Description :

~~~
Desperate times need desperate solutions. Every legendary Elven warrior spirit was doomed to dwell inside cold colossal statues. The damnation to continue war against uncivilized aliens once again.

FEATURES:
- separate torsos and legs;
- 5 separate heavy weapon back vanes.

UPDATE:
2024 May 14th - separate legs for Macrowraith models added, models are upscaled.

NOTICE:
- the models are not tested yet, there’re no supported stls required for safe printing, so the prints are required for experienced hobbyists;
- model sturdiness is questionable (the hip is 1.2mm thick);
- the preview image is dated.

THANKS TO:
- PopsicleBallista for original models ( https://cults3d.com/en/3d-model/game/grim_humptydumpty_robot );
- Lord_Chronos for permission to use his works for free sharing ( https://cults3d.com/en/users/Lord_Chronos/3d-models )
~~~

Justification du tag SUP :

- the models are not tested yet, there’re no supported stls required for safe printing, so the prints are required for experienced hobbyists;





