
---
title: "[BON-MARCHE] Cults3d_supprimes > BitGloomMinis > Epic_Scale_Trench_pieces [non dl]"
date: "2023-04-16T12:30:15Z"
toc: true
image: "thumbs_-_epic-scale-trench-pieces_-_Images_1.jpg"
tags:
- "BitGloomMinis"
- "epic"
- "decors"
- "epic decors"
- "IMP"
- "epic IMP"
- "decors IMP"
- "epic decors IMP"
- "Barricades"
- "epic Barricades"
- "decors Barricades"
- "epic decors Barricades"
- "IMP Barricades"
- "epic IMP Barricades"
- "decors IMP Barricades"
- "epic decors IMP Barricades"
- "NONDL"
- "epic NONDL"
- "decors NONDL"
- "epic decors NONDL"
- "IMP NONDL"
- "epic IMP NONDL"
- "decors IMP NONDL"
- "epic decors IMP NONDL"
- "Barricades NONDL"
- "epic Barricades NONDL"
- "decors Barricades NONDL"
- "epic decors Barricades NONDL"
- "IMP Barricades NONDL"
- "epic IMP Barricades NONDL"
- "decors IMP Barricades NONDL"
- "NONSUP"
- "epic NONSUP"
- "decors NONSUP"
- "epic decors NONSUP"
- "IMP NONSUP"
- "epic IMP NONSUP"
- "decors IMP NONSUP"
- "epic decors IMP NONSUP"
- "Barricades NONSUP"
- "epic Barricades NONSUP"
- "decors Barricades NONSUP"
- "epic decors Barricades NONSUP"
- "IMP Barricades NONSUP"
- "epic IMP Barricades NONSUP"
- "decors IMP Barricades NONSUP"
- "NONDL NONSUP"
- "epic NONDL NONSUP"
- "decors NONDL NONSUP"
- "epic decors NONDL NONSUP"
- "IMP NONDL NONSUP"
- "epic IMP NONDL NONSUP"
- "decors IMP NONDL NONSUP"
- "Barricades NONDL NONSUP"
- "epic Barricades NONDL NONSUP"
- "decors Barricades NONDL NONSUP"
- "IMP Barricades NONDL NONSUP"

---

![](<thumbs_-_epic-scale-trench-pieces_-_Images_1.jpg> "thumbs_-_epic-scale-trench-pieces_-_Images_1.jpg")

  

Source : [https://cults3d.com/en/3d-model/game/epic-scale-trench-pieces](https://cults3d.com/en/3d-model/game/epic-scale-trench-pieces)

Id objet : epic-scale-trench-pieces

Prix : 0.99 euros

Collections :

- epic decors IMP Barricades


Collections pour tags :

- epic decors IMP Barricades NONDL NONSUP


Description :

~~~
A set of trench pieces scaled for epic/6-8mm. This set is designed to be used for gameplay purposes, so the trenches are large enough to fit 20mm wide bases in them easily.

Use them to represent entrenchments and fortifications. The source file is also included so you can also make your own adjustments or your own trenches.

Included - 

2 straight trench pieces
1 trench exit ramp piece
1 trench with a dugout
1 T-junction piece
1 corner piece
1 slanted piece
1 trench with a vision slit
2 "pits", 1 with a raised platform
1 Blender file with all pieces

Enjoy! 

*Update* It was reported that some pieces weren't flat so were a little harder to print flat to the base, so I endeavoured to make all pieces flat.
~~~




